...
export class MobileTableRenderer {
  ...
  async createTableRow(
    ...
    const rowWidth = totalColumnWidth + paddingLeft + paddingRight + totalItemSpacing;
  ...
  async createMobileTableHeader(
    id: number,
    createHeaderText: (text: string, width: number) => Promise<FrameNode>,
    _createCellText: (text: string, width: number, bold?: boolean) => Promise<FrameNode>
  ): Promise<FrameNode> {
    ...
    const [, iosWidth, androidWidth] = columnWidths; // Skip label column
    if (iosWidth === undefined || androidWidth === undefined) {
      throw new Error("Missing iOS or Android width from columnWidths");
    }
    const iosHeader = headerTexts[0];
    const androidHeader = headerTexts[1];
    if (!iosHeader || !androidHeader) {
      throw new Error("Missing iOS or Android header text");
    }
    row.appendChild(await createHeaderText(iosHeader, iosWidth));
    row.appendChild(await createHeaderText(androidHeader, androidWidth));
